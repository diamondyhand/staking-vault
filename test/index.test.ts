  ...
  describe("Test Start.", () => {
    describe("Main Functions Start.", () => {
      it("lock increaselock unlock", async () => {
        ...
        console.log("Tom's reward is ", await StakingVault.getClaimableRewards(Tom.address));
        ...
        // await StakingVault.connect(Tom).unlock(1000, 60 * dayTime);

        console.log("Vault is ", await StakingToken.balanceOf(StakingVault.address));
      })

      it("lock increaselock unlock", async () => {
        await StakingVault.setRewardDistributor(Jerry.address);
        await StakingToken.mint(Jerry.address, 10000);
        await StakingToken.connect(Jerry).approve(StakingVault.address, 1000);
        // await StakingVault.connect(Jerry).notifyRewardAmount(1000);
        // console.log("Tom's reward is ", await StakingVault.getClaimableRewards(Tom.address));
      })
    });
  });
